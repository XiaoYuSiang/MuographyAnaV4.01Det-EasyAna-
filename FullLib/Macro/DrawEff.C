...
const double EffSet = 1.2;
const double EffBound[2] = {0.6,1.0};
void DrawEff(  const int index = 28)
{
  ...
  for(int iSE =0; iSE<NSe;iSE++){
    ...
    TH1F *hAllEff2 = new TH1F("hAllEff2","All Eff. fit;General Ch.ID;Ratio",NumBD*NumCh,0,NumBD*NumCh);
    TH2F *hEff[2], *hSum[2];
    for(int i=0;i<NumLY-2;i++){
      string Target = Form("iY0-DiY*%d:iX0-DiX*%d",i+1,i+1);
      c1->cd((i)*NGpx+1);
    ...
    for(int iBD=0;iBD<NumBD;iBD++){
      ...
      h1[iBD]->GetYaxis()->SetRangeUser(EffBound[0],EffBound[1]);
      h1[iBD]->SetStats(0);
    }
    for(int iLay=1;iLay<NumLY-1;iLay++){
      ...
      for(int iBinX = 1;iBinX<=nbinsX;iBinX++){
        for(int iBinY = 1;iBinY<=nbinsY;iBinY++){
          ...
          if(PosTable[ix][iy][iLay].Alive){
            ...
            hAllEff2->SetBinContent(PosTable[ix][iy][iLay].GCH+1,Eff);
            hAllEff2->SetBinError(PosTable[ix][iy][iLay].GCH+1,Err);
          }
        }
      }
    ...
    cout<<188<<" All = "<<hAllEff2->Integral(-1,-1)<<endl;
    ...
    hAllEff2->GetXaxis()->SetRangeUser(NumCh*LayFirst[1],NumCh*(LayFirst[NumLY-1])-1);
    hAllEff2->GetYaxis()->SetRangeUser(EffBound[0],EffBound[1]);
    hAllEff2->Draw("pe");
    TF1 *tf1tmp = (TF1*) hAllEff2->GetFunction("gaus");
    hEffs[iSE] = (TH1F*) hAllEff2->Clone();
    hEffs[iSE]->SetName(Form("hSE%d",iSE));
    // tf1tmp->Draw("lsame");
    ...
    c2->Close();
    cout<<231<<endl;
    for(int i=0;i<NumBD;i++) delete h1[i];
  }
  
  
  hEffs[NSe] = (TH1F*) hEffs[0]->Clone();
  hEffs[NSe]->Divide(hEffs[1]);
  hEffs[NSe]->GetYaxis()->SetRangeUser(0,2);
  hEffs[NSe]->SetName("hEffDiv");
  hEffs[NSe]->Draw("he");
  
  c1->Print((DirResultEff+"Eff2MethodCMP.gif").data());
